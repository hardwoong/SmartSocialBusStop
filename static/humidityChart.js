...
let config = {
	...
	data: {
		...
		datasets: [{
			label: '습도 센서로부터 측정된 실시간 습도', // 데이터셋 레이블
			backgroundColor: 'rgba(135, 206, 235, 0.2)', // 그래프 배경 색상 (연한 하늘색)
			borderColor: 'rgb(30, 144, 255)', // 그래프 선 색상 (도저블루)
	...
	options: {
		...
		scales: { // x축과 y축 정보
			...
			yAxes: [{
				...
				scaleLabel: { display: true, labelString: '습도(%)' }, // y축 레이블
				ticks: {
					min: 0, // y축 최소값 설정
					max: 90 // y축 최대값 설정
				}
...
function addChartData(value) {
	...
	if(value < 20) { // 습도가 20% 미만인 경우
		status.innerHTML = "건조"; // 상태를 '건조'로 설정
		status.style.color = "red"; // 텍스트 색상을 빨간색으로 설정
	}
	else if(value >= 20 && value <= 50) { // 습도가 20% 이상 50% 이하인 경우
		status.innerHTML = "보통"; // 상태를 '보통'으로 설정
		status.style.color = "green"; // 텍스트 색상을 초록색으로 설정
	}
	else { // 습도가 50%보다 큰 경우
		status.innerHTML = "습함"; // 상태를 '습함'으로 설정
