...
let config = {
	...
	data: {
		...
		datasets: [{
			label: '조도 센서로부터 측정된 실시간 조도', // 데이터셋 레이블
			backgroundColor: 'rgba(255, 215, 0, 0.2)', // 그래프 배경 색상 (연한 골드색)
			borderColor: 'rgb(255, 165, 0)', // 그래프 선 색상 (오렌지색)
	...
	options: {
		...
		scales: { // x축과 y축 정보
			...
			yAxes: [{
				...
				scaleLabel: { display: true, labelString: '조도' }, // y축 레이블
				ticks: {
					min: 0, // y축 최소값 설정
					max: 1000 // y축 최대값 설정
				}
...
function addChartData(value) {
	...
	if(value < 100) { // 조도가 100 미만인 경우
		status.innerHTML = "어두움"; // 상태를 '어두움'으로 설정
		status.style.color = "black"; // 텍스트 색상을 검은색으로 설정
	}
	else if(value >= 100 && value <= 300) { // 조도가 100 이상 300 이하인 경우
		status.innerHTML = "보통"; // 상태를 '보통'으로 설정
		status.style.color = "orange"; // 텍스트 색상을 오렌지색으로 설정
	}
	else { // 조도가 300보다 큰 경우
		status.innerHTML = "매우 밝음"; // 상태를 '매우 밝음'으로 설정
